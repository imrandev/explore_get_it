    ...
    final databaseOptions = sqflite.OpenDatabaseOptions(
      ...
      onCreate: (database, version) async {
        await database.execute(
            'CREATE TABLE IF NOT EXISTS `Note` (`id` INTEGER PRIMARY KEY AUTOINCREMENT, `title` TEXT, `description` TEXT, `createdDate` TEXT)');
...
class _$NoteDao extends NoteDao {
  ...
  Future<Note?> delete(int id) async {
    return _queryAdapter.query('DELETE FROM Note WHERE id = ?1',
        mapper: (Map<String, Object?> row) => Note(
            row['id'] as int?,
            row['title'] as String?,
            row['description'] as String?,
  ...
  Future<int?> getCount() async {
    return _queryAdapter.query('SELECT COUNT (*) FROM Note',
